...
In this scenario, \(r\) ends up with \(q\), which is strictly better under \(r\)'s true preference. Thus, by lying about the order of two less preferred programs, \(r\) can secure a more-preferred match.
...
= x \log_2 n.
\]
Clearly, for large \(n\), \(x \log_2 n\) outgrows \(\sqrt{\log_2 n}\). Hence \(2^{\sqrt{\log_2 n}}\) is sub-polynomial}. It follows that:
\[
f_2(n) \;\ll\; n^x 
\quad
\text{and thus}
\quad
f_2(n) \;\ll\; f_1(n),\; f_4(n)
\]
...
\]
As \(n\to\infty\), \(n^{\tfrac{1}{3}}\) eventually outgrows \((\log n)^3\). Thus 
\[
f_1(n) \;\ll\; f_4(n).
\]
