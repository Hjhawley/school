...
// We don't need to explicitly return here
// Another example:
let max = if x > y {
    x
} else {
...
// In Rust, if-else can be used as an expression that returns a value,
// which is why you can assign it directly to a variable like max.
// Just make sure that x and y are of the same type and that the type of max
// is compatible with the type of x and y.
...
fn main() {
    ...
    // Immutable bindings (technically not a "variable" but often called that)
    let x: i32 = 1;
    ...
    // In this case it's redundant but sometimes it can be helpful.
    // This isn't CONVERTING one type to another, just CLARIFYING the type.
    ...
    // That ! means it's a macro, not a function; the code is actually rewritten at compile time
    // You could use a string literal as an argument, doesn't have to be a variable / binding
    ...
    // Capital S "Strings" are more expensive but more flexible.
