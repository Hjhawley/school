...
impl fmt::Display for Exp {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Exp::Let1 { var, value, body } => write!(f, "(let {var} {value} {body})"),
            Exp::Rec { var, var_type, value, body } => write!(f, "(rec ({var}: {var_type}) {value} {body})"),
            Exp::Lam {
                var,
                var_type,
                body,
            } => write!(f, "(lambda ({var}: {var_type}) {body})"),
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Value::Fun {
                ...
                body,
                nv,
            } => write!(f, "closure((lambda ({var}: {var_type}) {body}), {nv:?})"),
        }
    }
}

type Env = HashMap<String, Rc<RefCell<Option<Value>>>>;
...
fn main() {
    ...
    loop {
        ...
        loop {
            ...
            // scan the token list and count total nexting level
