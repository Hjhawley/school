...
import { drawRectangle, Rectangle } from "./rectangle.js";
import { drawCircle, Circle } from "./circle.js";
...
async function main() {
	...
	function click(event){
		...
		for(let i=0; i<circleList.length; i++)
		{
			const distanceSquared = (xWorld-circleList[i].x)**2 + (yWorld-circleList[i].y)**2;
			if (distanceSquared < circleList[i].size**2) {
				circleList[i] = circleList[circleList.length-1];
				circleList.pop();
				popSound.play();
				popped = true;
				break;
		...
		if(!popped){
			const size=randomDouble(1,2);
			AddNewCircle(xWorld,yWorld,size)
		}
	}
	//
	// Main render loop
